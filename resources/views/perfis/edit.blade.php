...
@section('content')      
    <div class="box box-primary">
        <div class="register-box-body">
            <h3 class="box-title text-center">Alteração dos dados de {{ $perfil->nome }}</h3>
            @include('errors.list')
            {!! Form::open(['url' => URL::to("perfis/$perfil->id_perfil")]) !!}
            {!! method_field('PUT') !!}
            {!! Form::token() !!}
                
                <div class="form-group has-feedback{{ $errors->has('nome') ? ' has-error' : '' }}">
                    {!! Form::text('nome', $perfil->nome, ['class'=>'form-control', 'maxlength'=>150, 'placeholder'=>'Nome']) !!}                    
                      @if ($errors->has('nome'))
